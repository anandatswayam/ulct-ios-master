//
//  GeneralInfoUsSenateViewController.m
//  ultc
//
...
@interface GeneralInfoUsSenateViewController ()
...
@implementation GeneralInfoUsSenateViewController
...
- (void)viewDidLoad
{
    ...
    if([s next]){
        ...
        NSArray * textFields = @[@"us_senate_address",
                                 @"us_senate_city",
                                 @"us_senate_state",
                                 @"us_senate_zip",
                                 @"us_senate_phone",
        ...
        if( ! [(NSString *) [self.dict objectForKey:@"us_senate_address"] isEqual:@""] ) {
            [self.fieldOrder addObject:@"map"];
            [self.fieldOrder addObject:@"us_senate_address"];
        }
        
        if( ! [(NSString *) [self.dict objectForKey:@"us_senate_phone"] isEqual:@""] ) {
            [self.fieldOrder addObject:@"us_senate_phone"];
        }
        
        if( ! [(NSString *) [self.dict objectForKey:@"us_senate_fax"] isEqual:@""] ) {
            [self.fieldOrder addObject:@"us_senate_fax"];
        }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([field isEqual:@"map"]){
        ...
        NSString * address = [NSString stringWithFormat:@"%@, %@, %@ %@",
                              [self.dict objectForKey:@"us_senate_address"],
                              [self.dict objectForKey:@"us_senate_city"],
                              [self.dict objectForKey:@"us_senate_state"],
                              [self.dict objectForKey:@"us_senate_zip"]
                              ];
    ...
    } else if([field isEqual:@"us_senate_address"]){
        ...
        addressCell.addressLine1.text = [self.dict objectForKey:@"us_senate_address"];
    ...
    else if([field isEqual:@"us_senate_phone"]){
        ...
        singleCell.content.text = [self.dict objectForKey:@"us_senate_phone"];
        cell=singleCell;
        
    } else if([field isEqual:@"us_senate_fax"]){
        SingleLineTableViewCell * singleCell = [WRUtilities getViewFromNib:@"SingleLineTableViewCell" class:[SingleLineTableViewCell class]];
        singleCell.title.text = @"Fax:";
        singleCell.content.text = [self.dict objectForKey:@"us_senate_fax"];
        cell=singleCell;
